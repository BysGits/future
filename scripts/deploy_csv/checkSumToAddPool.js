...
async function main() {
    ...
    for (var i = 0; i < json.length; i++) {
        if (priceMap.get(json[i].SYMBOL)[1] === 0) {
            sum += priceMap.get(json[i].SYMBOL)[0] * json[i].UASSETPERPOOL;
        } else {
            sum += priceMap.get(json[i].SYMBOL)[1] * json[i].UASSETPERPOOL;
        }
    }

    console.log("Total amount of eurb needed: " + sum + " EURB");
