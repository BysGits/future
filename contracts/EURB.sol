...
contract EURB is Ownable, AccessControl, ERC20Burnable, Pausable {
    ...
    function isTransactionExcludedFromFee(
        address from_,
        address to_
    ) public view returns (bool) {
        if (
            isSenderExcludedFromFee[from_] ||
            isReceiverExcludedFromFee[to_] ||
            from_ == owner() ||
            from_ == _feeReceiver ||
            to_ == owner() ||
            to_ == _feeReceiver
    ...
    function freezeAccount(address _account) public {
        ...
        require(_account != owner(), "EURB: Can not freeze owner");
